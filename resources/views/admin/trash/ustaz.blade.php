...
<div class="container">
    <a href="{{ route('admin.ustaz') }}" class="btn mt-2 mb-2 btn-info">
        <i class="fa-solid fa-chevron-left" style="margin-right: 5px;"></i>
        Back
    </a>
    <div class="card">
        <div class="card-body">
            <h2>Delected Data Ustaz</h2>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons" style="text-align: center;">
                                    <button type="button" class="btn btn-sm btn-outline-success restore-btn" data-username="{{ $user->ustaz->username }}">
                                        <i class="fa-solid fa-rotate-left"></i>
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-danger hard-delete-btn" data-username="{{ $user->ustaz->username }}">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.restore-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Konfirmasi',
                    text: `Apakah Anda yakin ingin memulihkan ustaz dengan username ${username}?`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        fetch('{{ route("admin.restoreUstaz") }}', {
                                method: 'POST',
                                headers: {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire('Dipulihkan!', 'Data ustaz telah dipulihkan.', 'success')
                                        .then(() => {
                                            // Refresh halaman
                                            setTimeout(() => {
                                                location.reload();
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('selected-Restore-btn').addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: `Apakah Anda yakin ingin memulihkan ${selectedUsernames.length} ustaz?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Pulihkan',
                cancelButtonText: 'Batal'
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.hard-delete-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Konfirmasi',
                    text: `Apakah Anda yakin ingin menghapus ustaz dengan username ${username} secara permanen?`,
                    icon: 'warning',
                    showCancelButton: true,
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('bulk-Harddelete-btn').addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: `Apakah Anda yakin ingin menghapus ${selectedUsernames.length} ustaz secara permanen?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Hapus',
                cancelButtonText: 'Batal'
