...
<style>
    ...
    #ustazTable th:first-child.sorting_asc,
    #ustazTable th:first-child.sorting_desc,
    #ustazTable th:first-child.sorting {
        background-image: none !important;
        cursor: default !important;
    }

    /* Menambahkan jarak antara Show entries/Search dan tabel */
    #ustazTable_wrapper .dataTables_filter {
        margin-bottom: 20px;
...
<div class="container">
    <button type="button" class="btn btn-primary mt-2 mb-2" data-toggle="modal" data-target="#tambahUstaz">
        <i class="fas fa-plus" style="margin-right: 5px;"></i>
        Tambah Data
    </button>

    <!-- Modal Tambah Data Ustaz -->
    <div class="modal fade" id="tambahUstaz" tabindex="-1" aria-labelledby="modalTambahUstazLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTambahUstazLabel">Tambah Data Ustaz</h5>
                </div>
                <form id="formTambahUstaz" action="{{ route('admin.createUstaz') }}" method="POST">
                    @csrf
    ...
    <!-- Script Modal Tambah Data Ustaz -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Handle form reset when Cancel button is clicked
            document.getElementById('btnCancel').addEventListener('click', function() {
                document.getElementById('formTambahUstaz').reset();
            });

            // Initialize modal
            var myModalEl = document.getElementById('tambahUstaz');
            ...
            var form = document.getElementById('formTambahUstaz');
    ...
    <a href="{{ route('admin.trashUstaz') }}" class="btn btn-danger mt-2 mb-2">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="ustazTable" class="table table-hover table-bordered" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($ustaz as $user)
                        <!-- Data dummy -->
                        <tr>
                            <td style="padding-left: 18px !Important;"><input type="checkbox" class="select-checkbox"></td>
                            <td>{{ $user->ustaz->nama }}</td>
                            <td>{{ \Carbon\Carbon::parse($user->ustaz->tanggal_lahir)->format('d M Y') }}</td>
                            <td>{{ $user->ustaz->alamat }}</td>
                            <td>{{ $user->ustaz->gender }}</td>
                            <td>{{ $user->ustaz->no_telepon }}</td>
                            <td>{{ $user->ustaz->username }}</td>
                            <td>{{ $user->ustaz->email }}</td>
                            <td>
                                <div class="action-buttons" style="text-align: center;">
                                    <button data-bs-toggle="modal" data-bs-target="#editUstaz{{ $user->ustaz->id_ustaz }}"
                                    ...
                                    <!-- Modal Edit Data Ustaz -->
                                    <div class="modal fade" id="editUstaz{{ $user->ustaz->id_ustaz }}" tabindex="-1" aria-labelledby="modalEditUstazLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                        <div class="modal-dialog modal-lg modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="modalEditUstazLabel">Edit Data Ustaz: {{ $user->ustaz->nama }}</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <form id="formEditUstaz" action="{{ route('admin.updateUstaz', $user->ustaz->id_ustaz ) }}" method="POST">
                                                    @csrf
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            <div class="col-6" style="text-align: left;">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="nama" class="form-label">Nama</label>
                                                                    <input type="text" class="form-control" id="editNama" name="nama" placeholder="{{ $user->ustaz->nama }}">
                                                                </div>

                                                                <!-- Tanggal Lahir -->
                                                                <div class="mb-3">
                                                                    <label for="date" class="form-label">Tanggal Lahir</label>
                                                                    <input class="form-control" id="editDate" name="date" type="text" placeholder="{{ $user->ustaz->tanggal_lahir->format('d-m-Y') }}">
                                                                </div>

                                                                <!-- Alamat -->
                                                                <div class="mb-3">
                                                                    <label for="alamat" class="form-label">Alamat</label>
                                                                    <input type="text" class="form-control" id="editAlamat" name="alamat" placeholder="{{ $user->ustaz->alamat }}">
                                                                ...
                                                                <div class="mb-3">
                                                                    ...
                                                                    <select id="gender" name="gender" class="form-control" style="color: #b2b2b3!important;">
                                                                        <option value="" disabled selected>{{ $user->ustaz->gender ?? 'Pilih Jenis Kelamin' }}</option>
                                                                        <option style="color: black!important;" value="Laki-laki" {{ $user->ustaz->gender === 'Laki-laki' ? 'disabled' : '' }}>Laki-laki</option>
                                                                        <option style="color: black!important;" value="Perempuan" {{ $user->ustaz->gender === 'Perempuan' ? 'disabled' : '' }}>Perempuan</option>
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            <div class="col-6" style="text-align: left;">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="telp" class="form-label">No Telepon</label>
                                                                    <input type="text" class="form-control" id="editTelp" name="telp" placeholder="{{ $user->ustaz->no_telepon }}">
                                                                </div>

                                                                <!-- Username -->
                                                                <div class="mb-3">
                                                                    <label for="username" class="form-label">Username</label>
                                                                    <input type="text" class="form-control" id="editUsername" name="username" placeholder="{{ $user->ustaz->username }}">
                                                                </div>

                                                                <!-- Email -->
                                                                <div class="mb-3">
                                                                    <label for="email" class="form-label">Email</label>
                                                                    <input type="email" class="form-control" id="editEmail" name="email" placeholder="{{ $user->ustaz->email }}">
                                                                </div>
                                    ...
                                    <script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            ...
                                            document.getElementById('EditbtnCancel').addEventListener('click', function() {
                                                document.getElementById('formEditUstaz').reset();
                                            });
                                        });
                                    </script>

                                    <button type="button" class="btn btn-sm btn-outline-danger soft-delete-btn" data-username="{{ $user->ustaz->username }}">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.soft-delete-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Konfirmasi',
                    text: `Apakah Anda yakin ingin menghapus ustaz dengan username ${username}?`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        fetch('{{ route("admin.softDeleteUstaz") }}', {
                                method: 'POST',
                                headers: {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire('Dihapus!', 'Data ustaz telah dihapus.', 'success')
                                        .then(() => {
                                            // Refresh halaman
                                            setTimeout(() => {
                                                location.reload();
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('bulk-delete-btn').addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: `Apakah Anda yakin ingin menghapus ${selectedUsernames.length} ustaz?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Hapus',
                cancelButtonText: 'Batal'
...
<script>
    $(document).ready(function() {
        var table = $('#ustazTable').DataTable({
            paging: true,
            lengthChange: true,
            searching: true,
            ordering: true,
