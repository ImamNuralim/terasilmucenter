...
class UmumController extends Controller
{
    public function index($kategori = null)
    {
        ...
        // Ambil data umum yang terkait dengan pengguna yang sedang login
        $umum = $user->umum;
    ...
    public function profile()
    {
        ...
        // Format tanggal
        $created_at_formatted = \Carbon\Carbon::parse($umum->created_at)->translatedFormat('d F Y');
    ...
    public function updateProfile(Request $request)
    {
        ...
        $request->validate([
            ...
            'username' => [
                ...
                function ($attribute, $value, $fail) {
                    // Cek jika username sudah ada di tabel murid, users, umum dan ustaz
                    if (
        ...
        // Persiapkan array update untuk data umum
        $umumUpdateData = [];
        ...
        if ($request->filled('pendidikan')) {
            $umumUpdateData['pendidikan'] = $request->input('pendidikan');
        }

        if ($request->filled('pekerjaan')) {
            $umumUpdateData['pekerjaan'] = $request->input('pekerjaan');
        }
        ...
        //     $umumUpdateData['gambar'] = $this->uploadImage($request->file('image'), $umum->gambar);
        // }
        ...
        if ($request->hasFile('image')) {
            ...
            $umumUpdateData['gambar'] = base64_encode($imageData);
        }
    ...
    public function updateDeskripsi(Request $request)
    {
        ...
        if ($request->filled('deskripsi')) {
            $umumUpdateData['deskripsi'] = $request->input('deskripsi');
        }
        // Update data umum
        if (!empty($umumUpdateData)) {
            $umum->update($umumUpdateData);
