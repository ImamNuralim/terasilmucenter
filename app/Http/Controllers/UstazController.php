...
class UstazController extends Controller
{
    public function index($kategori = null)
    {
        ...
        // Ambil data ustaz yang terkait dengan pengguna yang sedang login
        $ustaz = $user->ustaz;
    ...
    public function profile()
    {
        ...
        // Format tanggal
        $created_at_formatted = \Carbon\Carbon::parse($ustaz->created_at)->translatedFormat('d F Y');
    ...
    public function updateProfile(Request $request)
    {
        ...
        // Persiapkan array update untuk data ustaz
        $ustazUpdateData = [];
        ...
        if ($request->filled('pendidikan')) {
            $ustazUpdateData['pendidikan'] = $request->input('pendidikan');
        }

        if ($request->filled('pekerjaan')) {
            $ustazUpdateData['pekerjaan'] = $request->input('pekerjaan');
        }
        ...
        //     $ustazUpdateData['gambar'] = $this->uploadImage($request->file('image'), $ustaz->gambar);
        // }
        ...
        if ($request->hasFile('image')) {
            ...
            $ustazUpdateData['gambar'] = base64_encode($imageData);
        }
    ...
    public function updateDeskripsi(Request $request)
    {
        ...
        if ($request->filled('deskripsi')) {
            $ustazUpdateData['deskripsi'] = $request->input('deskripsi');
        }
        // Update data ustaz
        if (!empty($ustazUpdateData)) {
            $ustaz->update($ustazUpdateData);
